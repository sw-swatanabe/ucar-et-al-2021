...
ylabel({'(F-F0)/F0 '},'FontSize',12);
    ...
    for n=1:p;
              plot(Pr_data{1, i}.b{1, n}.btraw,Pr_data{1, i}.b{1, n}.bdeltaGp,  colorstring(n),'LineWidth',2);
...
ylabel({'(F-F0)/F0'},'FontSize',11);
   % ylim([-0.6 yl2])
    ...
    for n=1:p;
             plot(Pr_data{1, l}.b{1, n}.btraw,Pr_data{1, l}.b{1, n}.bdeltaGp,  colorstring(n),'LineWidth',2);
             %plot(b{1, 1}.bt,b{1, n}.bRp, 'r','LineWidth',2);
    end
end
...
for i=1:l;
    ...
    annotation('textbox', txt_title,'String',{'dF/F binonly'},'FontSize',10,'LineStyle','none');%'Color',[2 0 0]);
